...
func update(delta: float) -> void:
	...
	for i in parent.stratagems_time_and_life.size():
		...
		if (last_tick_time <= stratagem_time_and_life.spawn_time and stratagem_time_and_life.spawn_time <= timer):
			...
			lines.append(LineInstance.new(new_line, i, get_best_track()))
	update_line(delta)
...
func update_line(delta: float) -> void:
	for i in lines.size():
		...
		line.position = Vector2(
			(size.x + line.DEFAULT_ICON_DIAMETER * line.scale.x) * (time_lived / time_data.life_time) - line.total_arrow_width_cache * line.scale.x,
