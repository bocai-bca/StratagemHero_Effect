		...
		StratagemHeroEffect.GameState.HighScores:
			...
			match (StratagemHeroEffect.high_scores_showing_type):
				0:
					...
					type_context += "[cell][color=yellow]" + (str(StratagemHeroEffect_SaveAccess.save_struct_in_memory.classic_high_level.reach_level) if StratagemHeroEffect_SaveAccess.save_struct_in_memory.classic_high_level.reach_level >= 0 else "--") + "[/color][/cell][cell]--[/cell][/table]"
				1:
